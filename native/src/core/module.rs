    ...
    fn inject_magisk_bins(&mut self) {
        self.customs.push(CustomModule::Dir {
            children: {
                ...
                children.insert_entry(
                    cstr!("/system/bin/magisk"),
                    &FsPathBuf::default().join(get_magisk_tmp()).join("magisk"),
                    false,
                );
                children.insert_entry(
                    cstr!("/system/bin/magiskpolicy"),
                    &FsPathBuf::default()
                        .join(get_magisk_tmp())
                        .join("magiskpolicy"),
                    false,
                );
                children.insert_entry(cstr!("/system/bin/su"), cstr!("./magisk"), true);
                children.insert_entry(cstr!("/system/bin/resetprop"), cstr!("./magisk"), true);
                children.insert_entry(cstr!("/system/bin/supolicy"), cstr!("./magiskpolicy"), true);
